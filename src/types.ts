...
export interface User {
  ...
  employeeName?: string;
  employeeRole?: 'Founder' | 'HR' | 'CEO' | 'Manager';
  ministryName?: string;
  companyName?: string;
  followers?: string[];
  following?: string[];
...
export interface EBook {
  ...
  authorName: string;
  description: string;
  category: 'book' | 'research_paper' | 'poem' | 'kahani' | 'natak' | 'lekh' | 'other';
  fileUrl?: string;
  content?: string; // For text-based entries like poems/lekh
  reactions: {
    impactful: string[]; // array of userIds
    motivational: string[];
    inspiring: string[];
  };
  reposts: {
...
export interface CalendarEvent {
  ...
  date: string;
  time?: string;
  type: 'academic' | 'exam' | 'holiday' | 'event' | 'personal';
  creatorId: string;
  creatorName: string;
  creatorRole: string;
  description: string;
...
export interface Event {
  ...
  organizer: string;
  organizerId: string;
  category: 'award_ceremony' | 'farewell' | 'annual_function' | 'national_event' | 'international_event' | 'celebration' | 'job_fair' | 'walk_in' | 'hackathon' | 'seminar' | 'webinar' | 'quiz' | 'competition';
  image?: string;
  registrationUrl?: string;
  createdAt: string;
}
