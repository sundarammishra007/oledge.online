...
export default function Profile({ user }: ProfileProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    employeeRole: user.employeeRole || 'Founder',
    ministryName: user.ministryName || '',
    companyName: user.companyName || '',
    profileBanner: user.profileBanner || 'None',
  });

  const BANNER_OPTIONS: Record<string, string[]> = {
    student: ['None', 'Hopeful', 'on campus', 'off campus'],
    college: ['None', 'Admission open', 'Admission closed', 'campus Event'],
    company: ['None', 'on Recuirement', 'off Recuirement', 'walk- ins', 'job- fairs', 'company Events'],
    govt: ['None', 'On Recuirement', 'off Recuirements', 'job- fairs', 'walk-ins', 'walk-in', 'Govt Events'],
    admin: ['None']
  };

  const getBannerColor = (banner: string) => {
    const b = banner.toLowerCase();
    if (b.includes('open') || b.includes('on')) return 'bg-emerald-500';
    if (b.includes('closed') || b.includes('off')) return 'bg-rose-500';
    if (b.includes('event')) return 'bg-indigo-500';
    if (b.includes('walk')) return 'bg-orange-500';
    if (b.includes('fair')) return 'bg-blue-500';
    if (b === 'hopeful') return 'bg-amber-500';
    return 'bg-slate-600';
  };
  ...
  const addPortfolioLink = () => {
    if (newPortfolioLink && !formData.portfolioLinks.includes(newPortfolioLink)) {
      setFormData(prev => ({ ...prev, portfolioLinks: [...prev.portfolioLinks, newPortfolioLink] }));
      setNewPortfolioLink('');
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 transition-colors duration-300">
      ...
      <div className="relative">
        <div className="h-48 md:h-64 bg-slate-100 dark:bg-slate-800 rounded-t-3xl overflow-hidden relative group">
          ...
          {formData.profileBanner && formData.profileBanner !== 'None' && (
            <div className="absolute top-0 right-0 p-4">
              <motion.div 
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                className={cn(
                  "px-6 py-2 rounded-bl-[2rem] rounded-tr-3xl text-white font-black uppercase tracking-[0.25em] text-[10px] shadow-2xl backdrop-blur-md",
                  getBannerColor(formData.profileBanner)
                )}
              >
            ...
            <div className="absolute inset-0 bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <div className="bg-white/90 backdrop-blur p-4 rounded-2xl shadow-xl w-full max-w-md mx-4 space-y-4">
                ...
                <div>
                  ...
                  <select 
                    className="w-full px-4 py-2 bg-slate-50 rounded-xl border-none focus:ring-2 focus:ring-blue-500 text-sm font-bold text-slate-900"
                    value={formData.profileBanner}
                    onChange={(e) => setFormData(prev => ({ ...prev, profileBanner: e.target.value }))}
                  >
                    {BANNER_OPTIONS[user.role]?.map(opt => (
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap gap-3">
                  {formData.portfolioLinks.map(link => (
                    <div key={link} className="px-4 py-2 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 text-xs font-black rounded-xl border border-orange-100 dark:border-orange-800/50 flex items-center gap-3 group">
                      <a href={link} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-2">
                        <ExternalLink className="w-3 h-3" />
                        {link.replace(/^https?:\/\/(www\.)?/, '').split('/')[0]}
                      </a>
                      {isEditing && (
                        <button onClick={() => removePortfolioLink(link)} className="hover:text-red-500 transition-colors">
                          <X className="w-3 h-3" />
                        </button>
                      )}
                    </div>
                ...
                {isEditing && (
                  <div className="flex gap-3">
                    <input 
                      ...
                      className="flex-1 p-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent rounded-2xl focus:border-orange-500 transition-all font-bold text-slate-900 dark:text-white"
                    ...
                    <button 
                      ...
                      className="w-14 h-14 bg-orange-600 text-white rounded-2xl flex items-center justify-center hover:bg-orange-700 transition-all shadow-lg shadow-orange-200"
                    >
                ...
                {isEditing && (
                  <div className="flex gap-3">
                    <input 
                      ...
                      className="flex-1 p-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent rounded-2xl focus:border-purple-500 transition-all font-bold text-slate-900 dark:text-white"
                      value={newSkill}
                    ...
                    <button 
                      onClick={addSkill}
                      className="w-14 h-14 bg-purple-600 text-white rounded-2xl flex items-center justify-center hover:bg-purple-700 transition-all shadow-lg"
                    >
                      <Plus className="w-7 h-7" />
                    </button>
                  </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm p-8 space-y-8 transition-colors duration-300">
              <div className="flex items-center justify-between">
                ...
                {isEditing && (
                  <button 
                    ...
                    className="w-10 h-10 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl flex items-center justify-center"
                  >
              ...
              <div className="grid gap-6">
                {formData.projects?.map((project, index) => (
                  <div key={index} className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-slate-800 relative group transition-all">
                    {isEditing && (
                      <button onClick={() => removeProject(index)} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-red-500">
                    ...
                    {project.link && (
                      <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-6 text-indigo-600 font-black text-xs">
        ...
        {showResume && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl overflow-y-auto">
            ...
            >
              ...
              <div className="absolute top-8 right-8 flex items-center gap-3 z-30 print:hidden">
                <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl p-4 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl flex items-center gap-6 mr-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-r border-slate-100 dark:border-slate-700 pr-6 mr-2">Customize Resume</span>
                  <div className="flex items-center gap-4">
                    {Object.entries(resumeSections).map(([key, value]) => (
                      <button
                        key={key}
                        onClick={() => setResumeSections(prev => ({ ...prev, [key]: !value }))}
                        className={cn(
                          "px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all",
                          value 
                            ? "bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg" 
                            : "bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-slate-600"
                        )}
                      >
                        {key}
                      </button>
              ...
              <div className="p-16 space-y-16 print:p-0 print:space-y-10 bg-white dark:bg-white text-slate-900" id="resume-content">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-6 text-slate-500 font-bold text-sm">
                      ...
                      {user.portfolioLinks?.map(link => (
                        <span key={link} className="flex items-center gap-2"><LinkIcon className="w-4 h-4 text-slate-900" /> {link.replace(/^https?:\/\/(www\.)?/, '')}</span>
                      ))}
                    </div>
                  </div>
                ...
                {/* Summary */}
                {user.summary && resumeSections.summary && (
                  <div className="space-y-4">
                ...
                <div className="grid md:grid-cols-12 gap-16">
                  <div className="md:col-span-8 space-y-16">
                    ...
                    {/* Volunteer */}
                    {user.volunteerExperience && user.volunteerExperience.length > 0 && resumeSections.volunteer && (
                      <div className="space-y-8">
