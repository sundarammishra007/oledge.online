...
export default function NoticeBoard({ user }: NoticeBoardProps) {
  ...
  const handleRepost = async (noticeId: string) => {
    try {
      ...
      alert('Notice republished on oledge!');
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `notices/${noticeId}`);
    }
  };
  ...
  const shareToSocial = (notice: Notice, platform: 'twitter' | 'linkedin' | 'whatsapp') => {
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1 uppercase tracking-widest text-[10px]">Official updates from Colleges, Companies & Govt</p>
          </div>
        </div>
      ...
      {(user.role === 'govt' || user.role === 'college' || user.role === 'company' || user.role === 'admin') && (
        ...
        >
          ...
          Post an official announcement (Colleges, Companies, Govt)...
        </button>
      )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative group">
                    <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-50 transition-all">
                      ...
                      {notice.reposts.length > 0 && <span className="mr-1">{notice.reposts.length}</span>}
                      Republish
                    </button>
                    <div className="absolute bottom-full right-0 mb-2 hidden group-hover:block bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 min-w-[200px] z-10 transition-all duration-200">
                      <button 
                      ...
                      >
                        <Megaphone className="w-3 h-3" /> Within oledge
                      </button>
                      <button 
                        onClick={() => shareToSocial(notice, 'twitter')}
                        className="w-full text-left px-4 py-2 text-xs font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-600 rounded-xl transition-all flex items-center gap-2"
                      >
                        <Share2 className="w-3 h-3" /> Share on X / Twitter
                      </button>
                      <button 
                        onClick={() => shareToSocial(notice, 'linkedin')}
                        className="w-full text-left px-4 py-2 text-xs font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-600 rounded-xl transition-all flex items-center gap-2"
                      >
                        <Share2 className="w-3 h-3" /> Share on LinkedIn
                      </button>
                      <button 
                        onClick={() => shareToSocial(notice, 'whatsapp')}
                        className="w-full text-left px-4 py-2 text-xs font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-600 rounded-xl transition-all flex items-center gap-2"
                      >
