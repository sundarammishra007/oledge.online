...
import { User, connect } from '../types';
...
export default function CircleConnects({ user }: CircleConnectsProps) {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, 'connects'),
      where('receiverId', '==', user.uid),
      where('status', '==', 'pending')
    );
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setPendingRequests(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as connect)));
    });
    return () => unsubscribe();
  }, [user.uid]);
  ...
  useEffect(() => {
    ...
    const fetchPeers = async () => {
      ...
      try {
        const q = query(collection(db, 'users'), where('uid', 'in', user.Peers));
        const snapshot = await getDocs(q);
        setPeers(snapshot.docs.map(doc => doc.data() as User));
      } catch (error) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const results = snapshot.docs
        ...
        .filter(u => u.uid !== user.uid && !user.Peers?.includes(u.uid));
  ...
  const sendconnect = async (targetUser: User) => {
    try {
      ...
      handleFirestoreError(error, OperationType.CREATE, 'connects');
    }
  };

  const respondToRequest = async (request: connect, status: 'accepted' | 'rejected') => {
    try {
      const requestRef = doc(db, 'connects', request.id);
      if (status === 'accepted') {
        await updateDoc(requestRef, { status: 'accepted' });
        
        // Add to both users' Peers lists
        const userRef = doc(db, 'users', user.uid);
        const senderRef = doc(db, 'users', request.senderId);
        
        await updateDoc(userRef, { Peers: arrayUnion(request.senderId) });
        await updateDoc(senderRef, { Peers: arrayUnion(user.uid) });
      } else {
        await deleteDoc(requestRef);
      }
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `connects/${request.id}`);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">circle connects</h2>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {/* Peers List */}
          <div className="space-y-4">
            <h3 className="text-lg font-black text-slate-900 px-2">Your Peers</h3>
            {Peers.length > 0 ? (
              ...
              <div className="text-center py-12 bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
                ...
                <p className="text-slate-400 font-medium">You haven't added any Peers yet</p>
              </div>
            )}
          </div>
        </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold">Wants to be Peers</p>
                    </div>
                  </div>
